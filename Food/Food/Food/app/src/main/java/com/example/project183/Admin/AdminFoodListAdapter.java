...
public class AdminFoodListAdapter extends RecyclerView.Adapter<AdminFoodListAdapter.viewholder> {
    ...
    public AdminFoodListAdapter(ArrayList<Foods> items) {
        this.items = items;
    }
    ...
    @Override
    public void onBindViewHolder(@NonNull AdminFoodListAdapter.viewholder holder, @SuppressLint("RecyclerView") int position) {
        holder.titleTxt.setText(items.get(position).getTitle());
        holder.timeTxt.setText(items.get(position).getTimeValue() + " Phút");
        holder.priceTxt.setText(String.format("%.0f VNĐ", items.get(position).getPrice()));
